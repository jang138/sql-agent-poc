...
8. **단위 처리 (중요)**:
   ...
   - 단위는 결과 해석 단계에서 처리됩니다
9. **컬럼명 특수문자 처리 (중요)**:
   - 괄호나 특수문자가 포함된 컬럼명은 반드시 큰따옴표로 감싸세요
   - 예: "행정구역별(읍면동)", "종사상지위별(2)", "연령계층별(5세)"
   - 올바른 예: WHERE "행정구역별(읍면동)" = '제주특별자치도'
   - 잘못된 예: WHERE 행정구역별(읍면동) = '제주특별자치도'
...
- 괄호가 포함된 컬럼명은 큰따옴표로 감싸세요 (예: "행정구역별(읍면동)")
- 모든 따옴표를 정확히 열고 닫으세요
- 세미콜론(;)으로 끝내세요

올바른 예:
SELECT 값 FROM housing_type_sido_stats WHERE "행정구역별(읍면동)" = '제주특별자치도' AND 년도 = '2019';
"""
